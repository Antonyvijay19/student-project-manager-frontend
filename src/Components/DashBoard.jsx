...
const DashBoard = () => {
  ...
  return (
    <div className='mb-2'>
      <h2 className="text-center text-5xl font-bold mb-10 text-red-700 mt-24">
        Projects 📚
      </h2>
      <div className='flex justify-center items-center gap-2'>
        <input
          ...
          className="w-full sm:max-w-md px-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:border-black"
        />
...

      {
        ...
        <button
          className='px-5 py-[10px] rounded-md font-bold text-center shadow-md text-white bg-gradient-to-l from-gray-600 to-gray-900 hover:from-red-700 hover:to-red-400' onClick={openPopup}>
          Add Project
        </button> 
  
        <button
          className='px-5 py-[10px] rounded-md font-bold text-center shadow-md text-white bg-gradient-to-l from-gray-600 to-gray-900 hover:from-red-700 hover:to-red-400' onClick={()=>{navigate("/invitestudent")}}>
          Invite Student
        </button>
        ...
        <div className="project-container grid grid-cols-1 md:grid-cols-2 gap-6 container mx-auto px-4 mt-5">
          {filteredProjects.map((project) => (
            <div key={project._id} className="blog-post mb-8 p-6 bg-gray-200 shadow-lg rounded-lg relative" >
              
              {
                log?
                <button className="bg-red-500 text-white px-4 py-2 rounded hover:bg-red-600 absolute top-3 right-3" onClick={() => navigate(`/editproject/${project._id}`)}>
                Edit
              </button> : null
              }
              ...
              <p className="text-sm project-startDate text-gray-800 sm:text-lg mb-4 font-normal"><span className='text-sm sm:text-lg font-normal text-gray-800'>Start Date:</span> {project.date}</p>

              <p className="text-sm project-deadline text-gray-800 sm:text-lg mb-4 font-normal"><span className='text-sm sm:text-lg font-normal text-gray-800'>Deadline:</span> {project.deadline}</p>
